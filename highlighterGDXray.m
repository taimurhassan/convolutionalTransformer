...
K = 3; % max number of instances
paths = {}; % path to each instance results
for i = 1:K
    paths{i} = ['C:\tcsvt\testingDataset\segmentation_results' num2str(i)];
end
...
pn= 'C:\tcsvt\testingDataset\segmentation_results1\'; % initial segmentation segmentation results path
pnO = 'C:\tcsvt\testingDataset\test_images\';
...
for i=1:nfile
    ...
    im = imread([pn fn]); % semantic segmentation results
    ...
    k = getCorrectInstance(imbinarize(rgb2gray(im)), im2, net);
    
    % valid instance
    if k > 1 && k < K
         pn = paths{k};
    else
         % (exception) if instance prediction is wrong,
         % choose the most recent instance model by default
         pn = paths{end}; 
    end
    ...
    saveas(gcf,['C:\tcsvt\results\gdxray\examples\' fn],'png');
end